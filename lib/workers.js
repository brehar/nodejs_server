...
const log = (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) => {
	...
	_logs.append(logFileName, logString, err => {
		if (err) {
			debug(`Logging to file for check with ID ${originalCheckData.id} failed.`);
		}
	});
};
...
const processCheckOutcome = (originalCheckData, checkOutcome) => {
	...
	_data.update('checks', newCheckData.id, newCheckData, err => {
		if (!err) {
			...
			debug(`Error trying to save updates to check with ID ${newCheckData.id}.`);
		}
	});
};
...
const validateCheckData = originalCheckData => {
	...
	if (
		...
		debug(`Error: Check with ID ${originalCheckData.id} is not properly formatted. Skipping.`);
	}
};
			...
			checks.forEach(check => {
				_data.read('checks', check, (err, originalCheckData) => {
					if (!err && originalCheckData) {
						...
						debug(`Error reading data for check ${check}: ${err}`);
					}
				});
			});
		} else {
			debug(`Error finding checks to process: ${err}`);
		}
	});
};
				...
				_logs.compress(logId, newFileId, err => {
					if (!err) {
						_logs.truncate(logId, err => {
							if (err) {
								debug(`Error truncating log with ID ${logId}: ${err}`);
							}
						});
					} else {
						debug(`Error compressing log with ID ${logId}: ${err}`);
					}
				});
			});
		} else {
			debug(`Could not find any logs to rotate: ${err}`);
		}
	});
};
